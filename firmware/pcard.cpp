...
//Definitions
#define CCUport Serial1    // communication port between CCU generator and the pCard
...
void setup() {
  ...
  //interupts-------------------------------------------------------------------------------------------------------------------------------------- Interrups
  attachInterrupt(digitalPinToInterrupt(Triggerpin), TriggerInterrup, FALLING);   // main trigger, if comes in and CCU confirms, a delayed trigger out will be created and counted
...
//digitalWrite(RSTpin,digitalRead(VoltageOKpin); // if negative 5 is gone! it will keep shift registes in LOW mode.

  // put your main code here, to run repeatedly:
if(Serial1.available()){
Serial1.readBytes(income, 32);
if (income[0]==66&&income[1]==105&&income[2]==116&&income[3]==128){  //it is a sequence
Sequence=income[4]<<8;
...
void TriggerInterrup(void) {                        //sequence will be added, main loop will perform the programming.
...
for (int i=0;i<8;i++){
  ...
  digitalWrite(Data1pin , bitRead(bitstream[SeqCount][1],i));   // responcible for channel 5to8
  digitalWrite(Data2pin,  bitRead(bitstream[SeqCount][3],i));  // resposible for chanels 13 16
  digitalWrite(Data3pin ,  bitRead(bitstream[SeqCount][5],i)); //21 24
  digitalWrite(Data4pin ,  bitRead(bitstream[SeqCount][7],i)); // 29 32
  digitalWrite(Data5pin ,  bitRead(bitstream[SeqCount][9],i)); //39 40
  digitalWrite(Data6pin ,  bitRead(bitstream[SeqCount][11],i));//45 48
  digitalWrite(Data7pin ,  bitRead(bitstream[SeqCount][13],i));//53 56
  digitalWrite(Data8pin ,  bitRead(bitstream[SeqCount][15],i));//61 64
...
for (int i=0;i<8;i++){
  digitalWrite(Data1pin , bitRead(bitstream[SeqCount][0],i)); //resposible for 1 to 4
  digitalWrite(Data2pin,  bitRead(bitstream[SeqCount][2],i)); // 9 to 12
  digitalWrite(Data3pin ,  bitRead(bitstream[SeqCount][4],i)); //17  20
  digitalWrite(Data4pin ,  bitRead(bitstream[SeqCount][6],i)); // 25  28
  digitalWrite(Data5pin ,  bitRead(bitstream[SeqCount][8],i)); //33 36
  digitalWrite(Data6pin ,  bitRead(bitstream[SeqCount][10],i));//41 44
  digitalWrite(Data7pin ,  bitRead(bitstream[SeqCount][12],i));//49 52
  digitalWrite(Data8pin ,  bitRead(bitstream[SeqCount][14],i));//57 60
  digitalWrite(CLKpin,HIGH);
//  delayMicroseconds(5);
  digitalWrite(CLKpin,LOW);
 // delayMicroseconds(5);
